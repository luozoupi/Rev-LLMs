...
class ImprovedReversibleBlock(nn.Module):
...
def init_reversible_model(model):
    """Proper initialization for reversible models"""
    
    def init_weights(m):
        if isinstance(m, nn.Linear):
            # Xavier initialization scaled down for reversible
    ...
    # Special initialization for reversible blocks
    for module in model.modules():
        if isinstance(module, ReversibleBlock):
            # Initialize gradient scale parameters
...
class ReversibleQwen3Model(nn.Module):
    """Full Qwen3 model with reversible layers and candidate selection"""
...
#     """Compare memory usage between standard and reversible models"""
    ...
    for name, attention_type, use_reversible in configs:
        ...
        try:
            ...
            model = create_reversible_qwen3_model(
                ...
                num_attention_heads=8,
                num_key_value_heads=4,
                # use_candidate_selection=use_candidate,
                attention_type=attention_type,
                use_reversible=use_reversible,
                reverse_thres=1024 if use_reversible else 999999
            )
            
            # ENHANCED FIX: Comprehensive device movement
            model = model.to(device)
            
            # # Force all parameters and buffers to the correct device
            ...
            # # Additional fix: explicitly move decoder layers if they exist
            # if hasattr(model.model, 'decoder_layers'):
            #     for layer in model.model.decoder_layers:
            #         layer = layer.to(device)
            #         move_to_device(layer, device)
            ...
            # # Debug: More comprehensive device check
            # print(f"  Model device check:")
            # sample_params = list(model.parameters())[:5]
            # for i, p in enumerate(sample_params):
            #     print(f"    Param {i} device: {p.device}")
            
            # # Check decoder layer devices specifically
            # if hasattr(model.model, 'decoder_layers') and len(model.model.decoder_layers) > 0:
            #     layer = model.model.decoder_layers[0]
            #     if hasattr(layer, 'input_layernorm'):
            #         print(f"    LayerNorm weight device: {layer.input_layernorm.weight.device}")
...
def test_model_configurations():
    """Test various model configurations to ensure robustness"""
    print("\n" + "="*60)
    ...
    test_configs = [
        # (name, hidden_size, num_heads, num_kv_heads, attention_type, use_reversible)
        ("Small Standard", 256, 8, 8, "standard", False),
        ("Small Reversible", 256, 8, 8, "standard", True),
        ("Small GQA", 256, 8, 4, "standard", True),
        ("Small Candidate", 256, 8, 4, "candidate_selection", True),
        ("Medium Standard", 512, 16, 8, "standard", True),
        ("Medium Candidate", 512, 16, 8, "candidate_selection", True),
    ]
    
    successful_configs = []
    failed_configs = []
    
    for name, hidden_size, num_heads, num_kv_heads, attention_type, use_reversible in test_configs:
        print(f"\nTesting {name}:")
        print(f"  hidden_size: {hidden_size}, heads: {num_heads}, kv_heads: {num_kv_heads}")
        print(f"  attention_type: {attention_type}, reversible: {use_reversible}")
        ...
        try:
            model = create_fixed_reversible_qwen3_model(
                ...
                attention_type=attention_type,
                use_reversible=use_reversible,
                reverse_thres=64 if use_reversible else 999999
            )
            
            model = model.to(device)
            
            # Quick forward pass test
            inputs = torch.randint(0, 500, (1, 64)).to(device)
            outputs = model(inputs)
            logits = outputs['logits'] if isinstance(outputs, dict) else outputs
            ...
            print(f"  ❌ Failed: {e}")
            failed_configs.append((name, str(e)))
    ...
    return len(successful_configs), len(failed_configs)
...
def quick_gradient_test():
    ...
    # Create test model with explicit parameters
    model = create_fixed_reversible_qwen3_model(
        vocab_size=1000,
        hidden_size=256,
        num_hidden_layers=2,
        num_attention_heads=8,
        num_key_value_heads=4,  # Explicitly set to 4 (must divide 8)
    ...
    try:
        ...
        print(f"✅ Gradients computed for {grad_params}/{total_params} parameters")
        
        # Test different attention types
        attention_types = ["candidate_selection"]
        
        for attention_type in attention_types:
            print(f"\nTesting {attention_type} attention...")
            try:
                test_model = create_fixed_reversible_qwen3_model(
                    vocab_size=1000,
                    hidden_size=256,
                    num_hidden_layers=2,
                    num_attention_heads=8,
                ...
                print(f"✅ {attention_type} attention working! Output shape: {test_logits.shape}")
...
if __name__ == "__main__":
    print("="*60)
    print("FIXED REVERSIBLE QWEN3 GRADIENT TEST")
    print("="*60)
    
    # Run gradient test first
    success = quick_gradient_test()
    
    if success:
        print("\n🎉 Basic gradient test passed!")
        
        # Run configuration tests
        successful, failed = test_model_configurations()
        
        print(f"\n" + "="*60)
        print("FINAL SUMMARY")
        print("="*60)
        print(f"✅ Basic gradient test: PASSED")
        print(f"✅ Configuration tests: {successful} passed, {failed} failed")
        
        if failed == 0:
            print("🎉 All tests passed! The fix is working correctly.")
        else:
            print("⚠️  Some configurations failed. Check logs above.")
            
    else:
        print("❌ Basic gradient test failed. Check the error above.")