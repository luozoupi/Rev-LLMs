...
class Enwik8Dataset(torch.utils.data.Dataset):
    """Fast character-level dataset for enwik8 with lazy loading"""
    
    def __init__(self, data, seq_length=512, stride=None):
        self.data = torch.from_numpy(data).long()
        self.seq_length = seq_length
        self.stride = stride or (seq_length // 4)  # Smaller stride for more sequences
        self.training = True
        
        # Calculate total number of sequences efficiently
        self.total_sequences = max(0, (len(self.data) - self.seq_length) // self.stride + 1)
        
        print(f"Enwik8Dataset: {len(self.data)} chars -> {self.total_sequences} sequences (stride={self.stride})")
        
    def __len__(self):
        return self.total_sequences
    
    def __getitem__(self, idx):
        # For training, use some randomness but keep it deterministic per epoch
        if self.training:
            # Add controlled randomness within stride boundaries
            base_idx = idx * self.stride
            max_offset = min(self.stride // 2, len(self.data) - self.seq_length - base_idx)
            if max_offset > 0:
                # Use deterministic randomness based on idx for reproducibility
        ...
        start_idx = min(start_idx, len(self.data) - self.seq_length - 1)
        
        # Get sequence and target efficiently
        seq = self.data[start_idx:start_idx + self.seq_length]
        target = self.data[start_idx + 1:start_idx + self.seq_length + 1]
...
class WikiTextDataset(torch.utils.data.Dataset):
    """Fast WikiText dataset with lazy sequence generation"""
    
    def __init__(self, dataset_split, tokenizer, seq_length=512, stride=None, max_samples=10000):
        ...
        # Pre-process and combine texts efficiently (like enhanced framework)
        print("Fast preprocessing WikiText data...")
        
        # Combine small texts into larger chunks for efficiency
        valid_texts = []
        current_text = ""
        ...
        print(f"Combined into {len(valid_texts)} text chunks")
        
        # Tokenize all texts efficiently
        print("Tokenizing combined texts...")
        self.all_tokens = []
        
        for text in tqdm(valid_texts, desc="Tokenizing", leave=False):
            try:
                tokens = tokenizer.encode(text.strip()).ids
                if len(tokens) > seq_length:  # Only keep texts with enough tokens
                    self.all_tokens.extend(tokens)
            except Exception:
                continue
        
        if len(self.all_tokens) < seq_length + 1:
            raise ValueError(f"Insufficient tokens: {len(self.all_tokens)} < {seq_length + 1}")
        
        # Calculate total sequences efficiently
        self.total_sequences = min(
            max_samples,
            (len(self.all_tokens) - self.seq_length) // self.stride + 1
        )
        
        print(f"WikiText dataset: {len(self.all_tokens)} tokens -> {self.total_sequences} sequences (stride={self.stride})")
    ...
    def __getitem__(self, idx):
        ...
        # Ensure we don't go out of bounds
        start_idx = min(start_idx, len(self.all_tokens) - self.seq_length - 1)
        
        # Get sequence efficiently
        tokens = self.all_tokens[start_idx:start_idx + self.seq_length + 1]
...
class ReversibleQwenPerformanceTester:
    ...
    def benchmark_dataset_loading_speed(self, dataset_type='wikitext', seq_len=512):
        """Benchmark the speed of optimized dataset loading"""
        import time
        
        print(f"\n{'='*60}")
        print(f"BENCHMARKING {dataset_type.upper()} DATASET LOADING SPEED")
        print(f"{'='*60}")
        
        if dataset_type == 'enwik8':
            ...
            start_time = time.time()
            sample_count = 0
            for i, (seq, target) in enumerate(train_dataset):
                sample_count += 1
                if i >= 1000:  # Test first 1000 samples
                    break
            iteration_time = time.time() - start_time
            print(f"Iteration time for {sample_count} samples: {iteration_time:.2f}s")
            print(f"Speed: {sample_count/iteration_time:.1f} samples/sec")
            
        elif dataset_type == 'wikitext':
            print("Loading WikiText data...")
            start_time = time.time()
            dataset = load_wikitext_data()
            tokenizer = create_wikitext_tokenizer(dataset, vocab_size=32000)
            load_time = time.time() - start_time
            print(f"Data loading + tokenizer time: {load_time:.2f}s")
            
            print("Creating optimized datasets...")
            start_time = time.time()
            train_dataset = WikiTextDataset(dataset['train'], tokenizer, seq_len, max_samples=8000)
            val_dataset = WikiTextDataset(dataset['validation'], tokenizer, seq_len, max_samples=1000)
            dataset_time = time.time() - start_time
            print(f"Dataset creation time: {dataset_time:.2f}s")
        ...
        return {
            'load_time': load_time,
            'dataset_time': dataset_time if 'dataset_time' in locals() else 0,
            'iteration_speed': sample_count/iteration_time if 'iteration_time' in locals() and iteration_time > 0 else 0
        }
    ...
    def run_comprehensive_test_enwik8(self, seq_len=512, use_enwik8=True, batch_size=256, num_workers: Optional[int] = None):
        ...
        if use_enwik8:
            try:
                ...
                # Optimized DataLoaders (disable multiprocessing to avoid tokenizer warnings)
                common = dict(
    ...
    def run_comprehensive_test(self, seq_len=512, vocab_size=32000, use_wikitext=True):
        ...
        if use_wikitext:
            try:
                ...
                print("Creating WikiText datasets with optimized processing...")
                train_dataset = WikiTextDataset(dataset['train'], tokenizer, seq_len, stride=seq_len//4, max_samples=8000)
                val_dataset = WikiTextDataset(dataset['validation'], tokenizer, seq_len, stride=seq_len//4, max_samples=1000) 
                test_dataset = WikiTextDataset(dataset['test'], tokenizer, seq_len, stride=seq_len//4, max_samples=1000)
                
                self.debug_dataset(train_dataset, "train")
                self.debug_dataset(val_dataset, "validation")
                self.debug_dataset(test_dataset, "test")
                
                print("Creating optimized WikiText data loaders...")
...
if __name__ == "__main__":
    ...
    print("🚀 Running OPTIMIZED Enhanced Performance Testing...")
    
    # First, benchmark the dataset loading speed improvements
    print("\n🔧 BENCHMARKING DATASET LOADING OPTIMIZATIONS")
    
    try:
        # Benchmark enwik8 loading speed
        print("Testing enwik8 optimizations...")
        enwik8_bench = tester.benchmark_dataset_loading_speed('enwik8', seq_len=512)
        print(f"Enwik8 loading benchmark: {enwik8_bench}")
    except Exception as e:
        print(f"Enwik8 benchmark failed: {e}")
        enwik8_bench = {'iteration_speed': 0}
    
    try:
        # Benchmark WikiText loading speed  
        print("Testing WikiText optimizations...")
        wikitext_bench = tester.benchmark_dataset_loading_speed('wikitext', seq_len=512)
        print(f"WikiText loading benchmark: {wikitext_bench}")
    except Exception as e:
        print(f"WikiText benchmark failed: {e}")
        wikitext_bench = {'iteration_speed': 0}
    
    print("\n🧪 Running ENHANCED ENWIK8 Character-Level Benchmarking...")
    ...
    print("="*50)
    print(f"  ✅ Enwik8 iteration speed: {enwik8_bench.get('iteration_speed', 0):.1f} samples/sec")
    print(f"  ✅ WikiText iteration speed: {wikitext_bench.get('iteration_speed', 0):.1f} samples/sec")
    print("  ✅ Lazy loading implemented for memory efficiency")
    print("  ✅ Vectorized operations for faster preprocessing")
    print("  ✅ Optimized DataLoader configurations")
    print("  ✅ Reduced memory footprint with on-demand sequence generation")
    print("  ✅ Better stride configuration for more training sequences")
    print("="*50)